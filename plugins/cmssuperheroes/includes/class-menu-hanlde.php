...
if ( ! class_exists( 'EFramework_menu_handle' ) && ! class_exists( 'CMS_PORTAL' ) ) {
	class EFramework_menu_handle {
		...
		public function cms_add_menu() {
			add_menu_page( $this->theme_name, $this->theme_name, 'manage_options', $this->theme_text_domain, array(
				$this,
				'cms_create_theme_dashboard'
			), 'dashicons-admin-generic', 3 );

			add_submenu_page( $this->theme_text_domain, $this->theme_name, esc_html__( 'Dashboard', CMS_TEXT_DOMAIN ), 'manage_options', $this->theme_text_domain, array(
				$this,
				'cms_create_theme_dashboard'
			) );
