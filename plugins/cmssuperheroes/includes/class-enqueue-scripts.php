...
if (!class_exists('EFramework_enqueue_scripts')) {
    class EFramework_enqueue_scripts
    {
        ...
        public function cms_admin_enqueue_scripts()
        {
            ...
            if (!empty($pagenow) && ($pagenow === 'post.php' && !empty($_REQUEST['post'])) || $pagenow === 'post-new.php') {
                ...
                if(!empty($post_format)){
                    wp_localize_script('post-format.js', 'post_format', [
        ...
        public function cms_front_enqueue_scripts(){
            wp_enqueue_script('cms-front-js',cmssuperheroes()->path('APP_URL') . '/assets/js/cms-front.js',array('jquery'),'all',true );
            wp_enqueue_script('cms-front-carousel-js',cmssuperheroes()->path('APP_URL') . '/assets/js/cms-front-carousel.js',array('jquery', 'owl-carousel'),'all',true );
        }
