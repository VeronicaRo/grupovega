...
class EF4_WPBakeryShortCode_VC_Media_Grid extends EF4_WPBakeryShortCode_VC_Basic_Grid {
    ...
    public function singleParamHtmlHolder( $param, $value ) {
        ...
        if ( 'include' === $param_name ) {
            ...
            foreach ( $images_ids as $image ) {
                ...
                $output .= ( $img ? '<li>' . $img['thumbnail'] . '</li>' : '<li><img width="150" height="150" test="' . $image . '" src="' . vc_asset_url( 'vc/blank.gif' ) . '" class="attachment-thumbnail" alt="'.esc_attr(get_bloginfo('name')).'" title="'.esc_attr(get_bloginfo('name')).'" /></li>' );
            }
            $output .= '</ul>';
            $output .= '<a href="#" class="column_edit_trigger' . ( ! empty( $images_ids ) ? ' image-exists' : '' ) . '">' . esc_html__( 'Add images', 'biger' ) . '</a>';
