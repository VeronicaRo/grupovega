...
class EFramework_Mega_Menu_Walker extends Walker_Nav_Menu
{
    ...
    {
        $post = get_post($id);
        $content = do_shortcode(cms_shortcode_empty_paragraph_fix($post->post_content));
        $css = $this->get_vc_custom_css($id);
        ob_start();
        cms_move_to_head($css);
        $css_ = ob_get_clean();
        $megamenu = apply_filters('cms_enable_megamenu', false);
        if ($megamenu)
            return '<ul class="sub-menu sub-megamenu"><li><div class="container">' . $content . $css_ . '</div></li></ul>';
        else
            return false;
    }
