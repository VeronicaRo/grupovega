...
if ( ! class_exists( 'CMS_Carousel' ) ) {
	class CMS_Carousel {
		...
		function add_shortcode_cms_carousel( $atts, $contents = '' ) {
			...
			$atts_extra = apply_filters( 'cms_carousel_shortcode_atts', $atts_extra );
			$atts       = shortcode_atts( $atts_extra, $atts );
		...
		protected function cms_columnize_content( &$content ) {
			...
			preg_match_all( '@(\[vc_column_inner[^\]]+)@', $content, $matches_row );

			$modify = apply_filters('cms_modify_cms_carousel_output', false);
			if(!$modify) {
				if ( is_array( $matches_row[0] ) ) {
					foreach ( $matches_row[0] as $start_shortcode ) {
						$content = str_replace( $start_shortcode . ']', '', $content );
					}
				}
				preg_match_all( '@(\[vc_row_inner[^\]]+)@', $content, $matches_in );
				if ( is_array( $matches_in[0] ) ) {
					foreach ( $matches_in[0] as $start_shortcode ) {
						$content = str_replace( $start_shortcode . ']', '', $content );
					}
				}
				$content = str_replace( '[/vc_row_inner]', '', $content );
				$content = str_replace( '[/vc_column_inner]', '', $content );
				...
				$content = str_replace( '[/vc_row_inner]', '[/vc_row_inner]|cms|', $content );
			}
