...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'single-hentry' ); ?>>
    ...
    <div class="entry-featured <?php if ( ! class_exists( 'ReduxFrameworkInstances' ) ) { echo 'img-auto'; } ?>">
        <?php
        $gallery_list = explode(',', consultivo_get_post_format_value('post-gallery-images', '')); //Value is array of id image list
        if (has_post_format('gallery') && $gallery_list[0] ) : ?>
            <?php
            $light_box = consultivo_get_post_format_value('post-gallery-lightbox', '0'); //Value is string ("0" or "1")
        ...
        <?php elseif (has_post_format('quote')) : ?>
            <?php
            $quote_text = consultivo_get_post_format_value('post-quote-cite', ''); //Value is string
            echo esc_attr($quote_text);
            ?>
        <?php elseif (has_post_format('link')) : ?>
            <?php
            $link_pf = consultivo_get_post_format_value('post-link-url', '#');// Value is url
    ...
    <div class="entry-holder">
        ...
        <div class="entry-more">
            <a class="read-more" href="<?php echo esc_url( get_permalink()); ?>"><i class="fa fa-plus"></i> <?php echo esc_html__('Read More', 'consultivo')?></a>
        </div>
    </div>
</article><!-- #post -->