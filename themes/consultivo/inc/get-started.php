...
if (!class_exists('CMS_PORTAL')) {
    ...
    // Welcome page
    add_action('admin_menu', 'consultivo_add_welcome_page');
    function consultivo_add_welcome_page()
    {
        $current_theme = wp_get_theme();
        if (is_child_theme()) {
            $current_theme = $current_theme->parent();
        }
        add_submenu_page('themes.php', esc_html__('About', 'consultivo') . ' ' . $current_theme->get('Name'), esc_html__('About', 'consultivo') . ' ' . $current_theme->get('Name'), 'manage_options', "{$current_theme->get('TextDomain')}-welcome", 'consultivo_welcome_page');
    }

    function consultivo_welcome_page()
    {
        $current_theme = wp_get_theme();
        if (is_child_theme()) {
            $current_theme = $current_theme->parent();
            ...
            <div class="welcome-page-inner">
                <div class="welcome-page-content">
                    <div class="welcome-page-title">
                                <span>
                                    <?php esc_html_e('Welcome to', 'consultivo') ?>
                                </span>
                        <span>
                    ...
                    <div class="welcome-page-description">
                                <span>
                                    <?php esc_html_e('In order to continue, please install and activate', 'consultivo') ?>
                                </span>
                    ...
                    <div class="welcome-page-actions">
                        <?php if ($cms_portal && count($cms_portal) > 0): ?>
                            <button type="button" id="btn-cms-get-started"
                                    class="button button-primary btn-activate"><?php esc_html_e('Activate', 'consultivo') ?></button>
                        <?php else: ?>
                            <button type="button" id="btn-cms-get-started"
                                    class="button button-primary btn-install"><?php esc_html_e('Install', 'consultivo') ?></button>
                        <?php endif; ?>
                    ...
                    <div class="welcome-page-note font-italic">
                        ...
                        <span>
                                    <?php esc_html_e("CMS Portal Plugin will allow you to update theme, install required plugins", 'consultivo') ?>
                                </span>
                    </div>
                </div>
            </div>
    ...
    if (!function_exists('consultivo_admin_notice_get_started')) {
        add_action('admin_notices', 'consultivo_admin_notice_get_started');
        function consultivo_admin_notice_get_started()
        {
            $current_theme = wp_get_theme();
            if (is_child_theme()) {
                $current_theme = $current_theme->parent();
                ...
                <div class="gt-notice-inner">
                    ...
                    <div class="gt-notice-body">
                        <span class="gt-theme-author">
                            <?php echo esc_html__('By', 'consultivo') ?>
    ...
    add_action('wp_ajax_get_started', 'consultivo_get_started');
    add_action('wp_ajax_nopriv_get_started', 'consultivo_get_started');
    if (!function_exists('consultivo_get_started')) {
        function consultivo_get_started()
        {
            try {
                if (isset($_POST["activate"]) && !empty($_POST["activate"])) {
                    $result = [
                        'stt' => false,
                        'msg' => esc_html__('Plugin CMS Portal have not installed yet!', 'consultivo'),
                        'data' => [],
                    ];
                    ...
                    foreach ($installed_plugins_data as $installed_plugin_file => $installed_plugin_data) {
                        ...
                        if ($_installed_plugin_file[0] == 'cms-portal') {
                            ...
                            if (!is_null($active_result)) {
                                $result = [
                                    'stt' => false,
                                    'msg' => esc_html__('Fail to activate plugin CMS Portal!', 'consultivo'),
                                    'data' => [],
                                ];
                            } else {
                                $current_theme = wp_get_theme();
                                ...
                                $result = [
                                    'stt' => true,
                                    'msg' => esc_html__('Successfully!', 'consultivo'),
                                    'data' => [
                    ...
                    if (!isset($_POST["download_link"]) || empty($_POST["download_link"])) {
                        throw new Exception(__('Something went wrong!', 'consultivo'));
                    }
                    ...
                    foreach ($installed_plugins_data as $installed_plugin_file => $installed_plugin_data) {
                        ...
                        if ($_installed_plugin_file[0] == 'cms-portal') {
                            ...
                            if (!is_null($active_result)) {
                                $result = [
                                    'stt' => false,
                                    'msg' => esc_html__('Fail to activate plugin!', 'consultivo'),
                                    'data' => [],
                                ];
                            } else {
                                $current_theme = wp_get_theme();
                    ...
                    if (!$is_installed) {
                        ...
                        if (!$install_result) {
                            $result = [
                                'stt' => false,
                                'msg' => __('Fail to install plugin!', 'consultivo'),
                                'data' => [],
                            ];
                        } else {
                            $installed_plugins_data = get_plugins();
...
add_action('after_switch_theme', 'consultivo_redirect_to_welcome_page');
function consultivo_redirect_to_welcome_page()
{
    $current_theme = wp_get_theme();
    if (is_child_theme()) {
        $current_theme = $current_theme->parent();
