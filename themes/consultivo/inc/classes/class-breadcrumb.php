...
class CMS_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        $args = wp_parse_args( $args, array(
            'home_label'            => esc_html__( 'Home', 'consultivo' ),
            '404_label'             => esc_html__( 'Not Found', 'consultivo' ),
            'search_results_label'  => esc_html__( 'Search Results', 'consultivo' ),
            'entry_max_length'      => 3,
    ...
    function add_post_type_archive_entry()
    {
        ...
        if ( $post_type )
        {
            if($post_type->labels->name == 'Products'){
                $post_type->labels->name = 'Shop';
            }
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(esc_html__( 'Tag:', 'consultivo' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id);
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'consultivo' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'consultivo_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'consultivo_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
        ...
        if ( 'post' === $post_type )
        {
            ...
            if ( $cat_obj )
            {
                ...
                $this->add_entry( apply_filters('consultivo_breadcrumb_single_post_title',get_the_title( $post )), get_permalink( $post->ID ) );
            }
        ...
        if ( $post_object )
        {
            ...
            if($post_object->labels->name == 'Products'){
                $post_object->labels->name = 'Shop';
            }
         ...
         */
        $entries = apply_filters( 'consultivo_breadcrumb_single', $entries, $post );
